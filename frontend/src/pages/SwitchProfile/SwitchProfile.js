...
const BDSubnet = (props) => {
    ...
    async function handleSubmit(event){
        ...
        await api.post('/swprofile', {site_id: getSiteId, sw_id: getSwId}).then(response => {
    ...
    return (
        <div className={getStyle}>
            {props.header
            ?
                <div className="h5">Switch Profile</div>
            :   
                <Header title="Create Switch Profile"/>
            }
            <form onSubmit={handleSubmit}>
                ...
                <p className="mt-2 h6">Switch Profile ID</p>
                <input style={{width: "50%"}} placeholder="Gateway value" value={getSwId} onChange={e => setSwId(e.target.value)}/>
