...
const createNewNode =
  "json2yaml ../ansible/json/vars.json > ../ansible/yml/vars.yml && ansible-playbook -i ../ansible/yml/hosts ../ansible/yml/create_newnode.yml"; //converte JSON->YAML & EXECUTA COMANDO ANSIBLE
    ...
    try {
      ...
      if (NodeParam.name && NodeParam.nodeId && NodeParam.serial) {
        fs.writeFileSync(
          ...
          JSON.stringify({ name: NodeParam.name, node_id: NodeParam.nodeId, serial: NodeParam.serial }, null, 2)
        ...
        await exec(createNewNode, { cwd: __dirname }, (err, stdout, stderr) => {
          if (err) {
            ...
            return response
              .status(200)
              .json({ createdNodeMember: true, statusMessage: "Node Member created successfully" });
          }
        });
      } else {
