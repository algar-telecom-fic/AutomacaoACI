...
const json2yaml = 'json2yaml ../ansible/json/vars.json > ../ansible/yml/vars.yml && ansible-playbook -i ../ansible/yml/hosts ../ansible/yml/create_swprof.yml' //converte JSON->YAML & EXECUTA COMANDO ANSIBLE
            ...
            if(site_id && sw_id){
                ...
                await exec(json2yaml, {cwd: __dirname}, (err, stdout, stderr) => {
                    if (err){
                        ...
                        runCommand(cmds, cb);
                        return response.status(200).json({createdSwProfile: true, statusMessage: "Switch Profile created successfully"});
                    }
                });
            }else{
